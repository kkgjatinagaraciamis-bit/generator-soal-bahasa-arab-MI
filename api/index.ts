...
import { IdentityData, MainInputData } from '../src/types';

const app = express();
app.use(express.json());

// Create API client with tracking headers
...
// API route to handle generation
app.post('/api/generate', async (req, res) => {
  try {
    const { identity, mainInput } = req.body as { identity: IdentityData; mainInput: MainInputData };
    if (!identity || !mainInput) {
      return res.status(400).json({ error: 'Missing identity or mainInput data in request body' });
    }
    ...
    console.error('Gemini generation error in API:', error);
    res.status(500).json({ error: error?.message || 'Failed to generate questions. Please try again.' });
  }
});
